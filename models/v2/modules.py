...
class pre_net(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        x = Activation("relu")(x)
        x = Dropout(0.5)(x, training=self.training)
        x = self.dense2(x)
        x = Activation("relu")(x)
        x = Dropout(0.5)(x, training=self.training)
        return x
...
class CBHG(tf.keras.Model):
    def __init__(self, K, conv_dim, training=False):
        ...
        self.gru_fw = tf.keras.layers.GRU(
            128, zero_output_for_mask=True, return_sequences=True
        )
        self.gru_bw = tf.keras.layers.GRU(128, go_backwards=True, return_sequences=True)
    ...
    def call(
        ...
        x = tf.concat(
            [
                Activation("relu")(
                    self.bn(self.conv_bank[i](input_data)), training=self.training
                )
